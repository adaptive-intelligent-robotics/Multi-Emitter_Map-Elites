#ifndef QD_CONTAINER_GRID_V2_HPP
#define QD_CONTAINER_GRID_V2_HPP

namespace sferes {
    namespace qd {
        namespace container {
            template <typename Phen, typename Params> class Grid_V2 {
            public:
	      ...
	      std::tuple<bool, bool, double> add_v2(indiv_t i1)
                {
                    if (i1->fit().dead())
		      return {false,false,0.0}; // not added, not a new cell, no improvement
		    ...
		    if (!_array(behav_pos)){
		      _array(behav_pos) = i1;
		      return {true, true, i1->fit().value()}; // added, new cell, with improvement
		    }
		    else if ( (i1->fit().value() - _array(behav_pos)->fit().value()) > epsilon
			      || (fabs(i1->fit().value() - _array(behav_pos)->fit().value()) <= epsilon
				  && _dist_center(i1) < _dist_center(_array(behav_pos)))) {
		      double former_fit = _array(behav_pos)->fit().value();
		      _array(behav_pos) = i1;
		      return {true, false, i1->fit().value() - former_fit}; // added, not a new cell, with improvement (can be negative if we use the centre tolerance).
                    }
		    return {false,false,0.0}; // not added, not a new cell, no improvement
                }
