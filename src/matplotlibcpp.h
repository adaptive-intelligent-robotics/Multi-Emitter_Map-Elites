...
struct _interpreter {
    ...
    PyObject *s_python_function_listedcolormap;
...
#  endif
	...
	PyObject* colorsname  = PyString_FromString("matplotlib.colors");
        ...
        if (!pyplotname || !pylabname || !matplotlibname || !cmname || !colorsname) {
        ...
        PyObject* pltcolors = PyImport_Import(colorsname);
        Py_DECREF(colorsname);
        if (!pltcolors) { throw std::runtime_error("Error loading module matplotlib.colors!"); }
	...
	s_python_function_listedcolormap = PyObject_GetAttrString(pltcolors, "ListedColormap");
	    ...
	    || !s_python_function_listedcolormap
        ...
        if (   !PyFunction_Check(s_python_function_show)
	       ...
	       //|| !PyFunction_Check(s_python_function_listedcolormap)
...
PyObject* vectorVectorToListTuple_Double(const std::vector< std::vector< double > > &data) {
	PyObject* list = PyList_New( data.size() );
	//if (!tuple) throw logic_error("Unable to allocate memory for Python tuple");
	for (unsigned int i = 0; i < data.size(); i++) {
	  PyObject* subTuple = NULL;
	  subTuple = vectorToTuple_Double(data[i]);
	  PyList_SET_ITEM(list, i, subTuple);
	}
	return list;
}
...
PyObject* airl_colormap()
{
  ...
  PyObject* carray = get_2darray(color);
  PyObject* tuple = PyTuple_New( 1);
  PyTuple_SET_ITEM(tuple, 0, carray);


  PyObject* kwargs = PyDict_New();
  if(!PyCallable_Check(detail::_interpreter::get().s_python_function_listedcolormap))
    std::cout<<"error not callable"<<std::endl;
  
  PyObject* result = PyObject_CallObject(detail::_interpreter::get().s_python_function_listedcolormap, tuple);
  if(!result)
    {
      std::cout<<"error  creating colormap"<<std::endl;
      PyErr_Print();
    }
  Py_DECREF(kwargs);
  Py_DECREF(tuple);
...
bool scatter(const std::vector<NumericX>& x,
             const std::vector<NumericY>& y,
	     const double s, // The marker size in points**2
             const std::vector<std::vector<NumericC> >& c) 
{
    assert(x.size() == y.size());

    PyObject* xarray = get_array(x);
    PyObject* yarray = get_array(y);
    PyObject* carray = get_2darray(c);
    ...
    if (res != NULL) {
      ...
      Py_DECREF(detail::_interpreter::get().s_python_function_scatter);
	     ...
	     ) // The marker size in points**2
{
    ...
    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "s", PyLong_FromLong(s));
    PyDict_SetItemString(kwargs, "c", zarray);
 
    PyObject *python_colormap = airl_colormap();
    
    PyDict_SetItemString(kwargs, "cmap", python_colormap);
    ...
    PyObject* res  = PyObject_Call(detail::_interpreter::get().s_python_function_scatter, plot_args, kwargs);
    //auto res = airl_colormap();
    if(show_colormap)
      colorbar(res);

    Py_DECREF(plot_args);
    Py_DECREF(kwargs);
    Py_DECREF(python_colormap);
...
{
    PyObject *args = PyTuple_New(1);
    PyTuple_SetItem(args, 0, PyInt_FromLong(numClicks));
